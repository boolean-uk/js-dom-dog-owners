...
const yesOrNo = (isGoodDog) => {
  return isGoodDog ? "Yes!" : "No!";
};

const goodBad = (isGoodDog) => {
...
const createDog = (dogCard) => {
  ...
  button.innerText = "";
  let buttonText = document.createTextNode("Nature");
  button.append(buttonText);
  mainDogSection.append(button);

  button.addEventListener("click", () => {
    goodBadDog(dogCard);
    button.innerText = `${goodBad(dogCard.isGoodDog)}`;
    text.nodeValue = yesOrNo(dogCard.isGoodDog);
    buttonText.nodeValue = goodBad(dogCard.isGoodDog);
  });
};
